...
def built_in_sink_exercise():
    ...
    streaming_input_df = spark.readStream.schema(jsonSchema).option("maxFilesPerTrigger", 1).json('/Users/sampatbudankayala/PycharmProjects/UdacityStreaming/data_spark/data.json')   # Set the schema of the JSON data
...
# TODO use console for now
    query = streaming_count_df.writeStream.format("console").queryName("counts").outputMode("complete").start()
