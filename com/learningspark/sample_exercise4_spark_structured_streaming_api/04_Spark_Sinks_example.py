...
def built_in_sink_exercise():
    ...
    streaming_input_df = spark.readStream \
        .schema() \  # Set the schema of the JSON data
    .option() \  # Treat a sequence of files as a stream by picking one file at a time
    .json('data.txt')"""
...
query = streaming_count_df \
    .writeStream \
    .format("")  \  # memory = store in-memory table (for testing only in Spark 2.0)
