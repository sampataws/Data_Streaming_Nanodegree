...
To get Spark to actually run the map step, you need to use an "action".
 One available action is the collect method. The collect() method takes the results from all of the clusters and 
 "collects" them into a single list on the master node.
"""
...
"""
You do not always have to write a custom function for the map step. 
